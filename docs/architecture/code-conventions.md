...
- Three apps (ADR-0012): `client/src/worker/`, `client/src/contractor/`, `client/src/officer/`. A page lives in the app that owns it. Code more than one app uses lives in `client/src/shared/`.
- Reuse components in `client/src/shared/components/` before adding a new one.
