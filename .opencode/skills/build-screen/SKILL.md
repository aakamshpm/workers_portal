...
3. Implement a thin page that calls `client/src/api.ts`. Add the API function if missing, typed to the contract.
4. Wire the route in `client/src/App.tsx` behind the existing `Guard`, on the correct role only.
5. Labour officer routes never get a hiring map or an install prompt.
